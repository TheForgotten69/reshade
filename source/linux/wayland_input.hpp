...
namespace reshade
{
	...
	public:
		...
		std::string clipboard_text() override { return _clipboard.text(); }
		void set_clipboard_text(const char *text) override { _clipboard.set_text(text, _last_serial); }
	...
	private:
		...
		wayland_clipboard _clipboard;
		wayland_scale_probe _scale_probe;
		double _preferred_scale = 0.0;
